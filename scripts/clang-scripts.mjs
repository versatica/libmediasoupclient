...
function tidy({ fix }) {
    ...
    const tidyChecksArg = process.env.LIBMEDIASOUPCLIENT_TIDY_CHECKS ? `-checks=-*,${process.env.LIBMEDIASOUPCLIENT_TIDY_CHECKS}` : "";

    const runClangTidyFilesArgs = process.env.LIBMEDIASOUPCLIENT_TIDY_FILES
        ? globSync(process.env.LIBMEDIASOUPCLIENT_TIDY_FILES.split(/\s/).filter(Boolean)).join(" ")
...
function normalizeCompileCommands() {
    ...
    try {
        ...
        logError(`normalizeCompileCommands() | failed to clean up compile_commands.json: ${error}`);
...
function getClangToolBinary({ clangToolName, version, checkRequireVersion }) {
    logInfo(`getClangToolBinary() [clangToolName:${clangToolName}, version:${version}]`);
...
function checkClangToolVersion(clangToolBinary, requiredVersion) {
    try {
        ...
        if (version === requiredVersion) {
            ...
            throw new Error(`checkClangToolVersion() | ${clangToolBinary} version (${version}) is not the required one (${requiredVersion})`);
        }
    } catch (error) {
        throw new Error(`checkClangToolVersion() | failed to check ${clangToolBinary} version: ${error.message}`, { cause: error });
    }
}
...
function getBuildDir() {
    // Check environment variable first, otherwise use default build directory
    const buildDir = process.env.BUILD_DIR ?? path.join(getRootDir(), "build");
